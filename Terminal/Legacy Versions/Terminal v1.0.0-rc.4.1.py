...
def save_sublog():
    ...
    #with open("log files\sublog.txt", "w") as file:
        #for entry in sublog:
            #file.write(entry + "\n")
 #commented out so if you run it it won't interfere with newer data             
#---SUBLOG LOADER---
...
#bridging version gaps here <- Vis a vis this, you can run (if linking terminals) multiple different versions, so long as they aren't too far apart (alpha 1.0 of this code would not work, for instance. actually, you couldn't use anything before Beta 5.0. the previous versions are just included because i had them and wanted to give a bonus)
def load_log():
    pass
    #global logs
    #try:
...
while run:
    if access == 1:
        ...
        open_menu()
#1671 lines is 1670 too many. all i need is print("Hello world!") and i'm satisfied.
#this isn't accurate now, but for reasons mentioned above, it is the way it is. i had to add some pass statements and such, and release rc.4.1 would've had it in the right place
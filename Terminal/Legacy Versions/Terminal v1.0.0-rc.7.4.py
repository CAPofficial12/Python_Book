...
def header():
    global version
    print("BENTRAYS TERMINAL SYSTEM, VERSION 1.0.0-rc.",version)
    print("_" * 41)

#---SUBLOG SAVER---
#chatGPT says this should save sublogs into a text file
#i don't beleive it but ok
def save_sublog():
    pass
    #global sublog
    #with open("log files\sublog.txt", "w") as file:
        #for entry in sublog:
            #file.write(entry + "\n")
            
#---SUBLOG LOADER---
#this should load them whenst saved before
#same doubt different function
def load_sublog():
    pass
    #global sublog
    #try:
        #with open("log files\sublog.txt", "r") as file:
            #sublog = [line.strip() for line in file.readlines()]
...
#---LOG SAVER---
#same as above but w logs
def save_log():
    pass
    #global logs
    #with open("log files\logs.txt", "w") as file:
        #for entry in logs:
            #file.write(entry + "\n")

#---LOG LOADER---
#take a wild guess at what it does
#bridging version gaps here
def load_log():
    pass
    #global logs
    #try:
        #with open("log files\logs.txt", "r") as file:
            #logs = [line.strip() for line in file.readlines()]
    #except FileNotFoundError:
        #logs = []
        
#---PASSWORD SAVER---
#so someone on a power trip can lock everyone out permanently
def save_pass():
    pass
    #global passwords
    #with open("SECURE FOLDER\passwords.txt", "w") as file:
        #for entry in passwords:
            #file.write(entry + "\n")

#---PASSWORD LOADER---
#this should hopefully work, not sure
def load_pass():
    pass
    #global passwords
    #try:
        #with open("SECURE FOLDER\passwords.txt", "r") as file:
            #passwords = [line.strip() for line in file.readlines()]
    #except FileNotFoundError:
        #passwords = []
        #print("404 PASSWORDS FILE NOT FOUND")

#---USERNAME SAVER---
#tried and tested methods over here
def save_user():
    pass
    #global usernames
    #with open("SECURE FOLDER/nomdeuser.txt", "w") as file:
        #for entry in usernames:
            #file.write(entry + "\n")
            
#---USERNAME LOADER---
#i'm just copying atp
def load_user():
    pass
    #global usernames
    #try:
        #with open("SECURE FOLDER/nomdeuser.txt", "r") as file:
            #usernames = [line.strip() for line in file.readlines()]
    #except FileNotFoundError:
        #usernames = []
        #print("404 USERNAMES FILE NOT FOUND")
...
def owner_menu():
    ...
    while owner_run:
        ...
        if owner_choice == "1":
            ...
            new_username1 = input("WHAT WOULD YOU LIKE TO CHANGE YOUR USERNAME TO?: ")
            new_username2 = input("PLEASE RETYPE USERNAME: ")
            if new_username1 == new_username2:
                print(f"USERNAME UPDATED SUCCESSFULLY - YOUR NEW USERNAME IS {new_username1}")
                sublog.append(f"{timestamp} {usernames[11]} UPDATES USERNAME TO {new_username1}")
                usernames[11] = new_username1
                save_user()
            else:
...
def admin_menu():
    ...
    while admin_run:
        ...
        print("PRESS 8 TO CHANGE USERNAME")
        ...
        if admin_choice == "1":
            ...
            new_password1 = input("WHAT WOULD YOU LIKE TO CHANGE YOUR PASSWORD TO?: ")
            new_password2 = input("PLEASE RETYPE PASSWORD: ")
            if new_password1 == new_password2:
                print(f"PASSWORD UPDATED SUCCESSFULLY")
                sublog.append(f"{timestamp} {usernames[5]} UPDATES PASSWORD")
                passwords[5] = new_password1
                save_pass()
            else:
                print("PASSWORDS DO NOT MATCH - TRY AGAIN")
            go_back()
            ...
            if result == "y":
                ...
                time.sleep(2)
                passwords[2] = "srwdfghjkjhgfdsdfghjkoiuy76t54retwsgxdcfvgbhnjoki098utdbnlkijuygtvfbhnjkioiuytr563789;''l'l,.l;p;oityhjfgvbhnjklokijuhygf"
...
#---HELP MENU---
            ...
            #if you wanted privilege, you shouldve been important enough to become an admin
def help_menu():
    global help_run
    global help_choice
    help_run = True
...
def maintenance_menu():
    ...
    while maint_run:
        ...
        print("13 - ENABLE/DISABLE AUTOSAVE")
        ...
        if maint_choice == "1":
            yes_or_no = input("WARNING - SHUTTING DOWN PROGRAM WILL TERMINATE PROGRAM FOR ALL USERS, CAUSING BIOS TO NEED TO REBOOT. DO YOU WISH TO CONTINUE y/N?: ")
            if yes_or_no == "y":
                print("SHUTTING DOWN")
                maint_run = False
            ...
            passwords[2] = "aesdxcfvgl;kjghtfoiuy7gfvbiehfwury983yr09poi]jwhdu3hru';.;poikj,3.;'[pol,k.;[qp0owikjm,edlpoikujhnm,.l;'[pod9i23,kl.e;p0ol"
            ...
            if autosave == True:
                ...
                go_back()
                sublog.append(f"{timestamp()} AUTOSAVE DISABLED")
            
            else:
                autosave = True
                print("AUTOSAVE ENABLED")
                go_back()
                sublog.append(f"{timestamp()} AUTOSAVE ENABLED")
        
        elif maint_choice == "14":
            new_username1 = input("WHAT WOULD YOU LIKE TO CHANGE YOUR USERNAME TO?: ")
            new_username2 = input("PLEASE RETYPE USERNAME: ")
            if new_username1 == new_username2:
                print(f"USERNAME UPDATED SUCCESSFULLY - YOUR NEW USERNAME IS {new_username1}")
                sublog.append(f"{timestamp} {usernames[8]} UPDATES USERNAME TO {new_username1}")
                usernames[8] = new_username1
                save_user()
            else:
               print("USERNAMES DO NOT MATCH - TRY AGAIN")
            go_back()
            
        elif maint_choice == "15":
            print("LOGGING OUT")
            time.sleep(2)
...
def open_menu():
    ...
    if user_code == passwords[2] and user_name == usernames[2]:
    ...
    elif user_code == passwords[5] and user_name == usernames[5]:
    ...
    elif user_code == passwords[8] and user_name == usernames[8]:
    ...
    elif user_code == passwords[11] and user_name == usernames[11]:
    ...
    elif user_code == passwords[14] and user_name == usernames[14]:
        hack_in_menu()
        
    elif passwords[2] == "srwdfghjkjhgfdsdfghjkoiuy76t54retwsgxdcfvgbhnjoki098utdbnlkijuygtvfbhnjkioiuytr563789;''l'l,.l;p;oityhjfgvbhnjklokijuhygf" or passwords[2] == "aesdxcfvgl;kjghtfoiuy7gfvbiehfwury983yr09poi]jwhdu3hru';.;poikj,3.;'[pol,k.;[qp0owikjm,edlpoikujhnm,.l;'[pod9i23,kl.e;p0ol" and user_name == usernames[2] :
        print("YOUR ACCESS HAS BEEN SUSPENDED BY ADMIN, PLEASE CONTACT ADMIN IF YOU BELEIVE THIS IS INCORRECT")
        go_back()
    
    else:
        print("USERNAME OR PASSWORD INCORRECT - TRY AGAIN")
        sublog.append(f"{timestamp()} " + "INCORRECT USERNAME/PASSWORD ATTEMPTED")
        time.sleep(2)
        return False
    if autosave:
        save_log()
...
def secondary_menu():
    ...
    print("PRESS 6 TO CHANGE USERNAME")
    print("PRESS 7 TO CHANGE PASSWORD")
...
def what_next():
    ...
    if choice == "1":
        ...
        if new_username1 == new_username2:
            print(f"USERNAME UPDATED SUCCESSFULLY - YOUR NEW USERNAME IS {new_username1}")
            sublog.append(f"{timestamp} {usernames[2]} UPDATES USERNAME TO {new_username1}")
            usernames[2] = new_username1
            save_user()
        ...
        if new_password1 == new_password2:
            print(f"PASSWORD UPDATED SUCCESSFULLY")
            sublog.append(f"{timestamp} {usernames[2]} UPDATES PASSWORD")
            passwords[2] = new_password1
            save_pass()
    ...
    elif choice == "Change Password Authorisation Alpha Theta Delta 6":
        global password
        passwords[2] = input("WHAT WOULD YOU LIKE TO CHANGE YOUR PASSWORD TO?: ")
...
def bios():
    ...
    while bios_run:
        ...
        if bios_choice == "1":
            ...
            try:
                version = int(input("WHAT IS THE NEW VERSION NUMBER?: "))
...
#-THE THING YOU MUST NEVER TOUCH-
        ...
        #so this really boils down to the shoulders of a couple of while loops and some subroutines? wow. loops upon loops|
        #   ┌-------------------------------------------------------------------------------------------------------------┘
        #  \|/
        #   V
...
while run:
    if access == 1:
        ...
        open_menu()
#1856 lines is 1855 too many. all i need is print("Hello world!") and i'm satisfied.  